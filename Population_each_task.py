...
try:
    ...
    sns.heatmap(matrix, annot=True, fmt="d", cmap="OrRd", linewidths=.5)
    # Note: "OrRd" is Orange-Red palette, distinct from the previous Blue-Green

    plt.title("Co-occurrence of GMFCS levels and functional tasks", fontsize=16)
    plt.ylabel("GMFCS level", fontsize=14)
    plt.xlabel("Functional tasks", fontsize=14)
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(r"C:\Users\bourgema\OneDrive - Université de Genève\Documents\ENABLE\Review\Plot\CO_occurence_gmfcs_task.png")
