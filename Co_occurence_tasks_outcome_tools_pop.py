...
try:
    ...
    # --- MODIFICATIONS : LABELS EN HAUT ET EN BAS EN MIROIR ---

    # 1. Graphique du HAUT (axes[0])
    axes[0].xaxis.tick_top()
    axes[0].tick_params(labeltop=True, labelbottom=False, length=0)
    # Inclinaison à 45 degrés, aligné à gauche
    plt.setp(axes[0].get_xticklabels(), rotation=45, ha='left', rotation_mode='anchor')

    # 2. Graphique du BAS (axes[-1] qui correspond au dernier graphique)
    axes[-1].xaxis.tick_bottom()
    axes[-1].tick_params(labelbottom=True, labeltop=False, length=0)
    # Inclinaison en miroir à -45 degrés, aligné à gauche
    plt.setp(axes[-1].get_xticklabels(), rotation=-45, ha='left', rotation_mode='anchor')

    # ----------------------------------------------------

    # Ajustement des marges (rect) pour laisser de la place en haut (0.90) ET en bas (0.10)
    plt.tight_layout(rect=[0.2, 0.10, 1, 0.90])
